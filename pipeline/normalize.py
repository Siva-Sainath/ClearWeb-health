...
# The 5 CPT codes we track for Hospital #1.
# normalize_file() will only retain rows matching these codes unless
# filter_codes=None is passed.
...
def normalize_file(
    ...
    if format_hint not in ("json", ""):
        logger.warning(
            f"[normalize] format_hint={format_hint!r} — expected 'json', proceeding anyway"
        )
        ...
        for entry in items:
            ...
            for charge in entry.get("standard_charges", []):
                ...
                for payer_info in charge.get("payers_information", []):
                    ...
                    payer = f"{payer_name} / {plan_name}".strip(" /")
