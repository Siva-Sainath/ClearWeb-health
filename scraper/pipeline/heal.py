...
def heal_collector(
    ...
    *,
    ...
    if p.returncode != 0 and not data:
        logger.warning("[heal] CLI heal failed (exit %s), trying REST refactor_template", p.returncode)
        try:
            from collectors.brightdata import refactor_template_rest

            refactor_template_rest(collector_id, reason)
            # Poll progress same as CLI stall handling
            api_url = f"https://api.brightdata.com/dca/collectors/{collector_id}/resume_automation_job"
            headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}
            requests.post(api_url, headers=headers, json={"message": True, "auto_save": True}, timeout=30)
            return {"status": "healed", "collector_id": collector_id, "via": "rest"}
        except Exception as rest_exc:
            if "rate_limited" in str(rest_exc):
                raise RuntimeError(f"rate_limited: {rest_exc}")
            raise RuntimeError(f"brightdata scraper heal failed (exit {p.returncode}); REST: {rest_exc}")
