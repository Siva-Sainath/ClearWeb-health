...
# Wide sane ranges for demo CPT codes not in Houston ground-truth set
GENERIC_CPT_RANGE = {"min_price": 25.0, "max_price": 150_000.0}


def expected_for_cpt_codes(cpt_codes: list[str]) -> dict[str, dict]:
    """Build validation spec for the patient's procedure CPT code(s)."""
    spec: dict[str, dict] = {}
    for code in cpt_codes:
        if code in EXPECTED_PROCEDURES:
            spec[code] = EXPECTED_PROCEDURES[code]
        else:
            spec[code] = {
                "name": f"CPT {code}",
                **GENERIC_CPT_RANGE,
