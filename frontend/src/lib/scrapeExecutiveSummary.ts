import type { FacilityResult, PatientProfile, PatientPriority, ScraperLog } from "@/lib/types";
import { facilityDialPhone, hasBookAction } from "@/lib/facilityContact";
...
export interface ScrapeExecutiveSummary {
  procedure: string;
  insurance: string;
  location: string;
  radiusMi: number;
  ...
  partialIssues: { name: string; issue: string }[];
  ranked: RankedOption[];
...
function withId(entry: FacilityResult, key: string): string {
  return entry.id ?? key;
}

function deriveWeights(priorities: PatientPriority[] = []): Record<string, number> {
...
function scoreFacility(
  f: FacilityResult,
  all: FacilityResult[],
  weights: ReturnType<typeof deriveWeights>
): number {
...
function buildReasons(f: FacilityResult, all: FacilityResult[], rank: number): string[] {
  ...
  const topRated = all.reduce((a, b) => (a.rating > b.rating ? a : b));

  reasons.push(`$${f.insurance_price} with your plan`);

  if (f.id === cheapest.id) {
    reasons.push("Lowest price in this search");
  } else if (rank === 1) {
    reasons.push("Best overall match for your priorities");
  }

  const distLabel = f.drive_min
    ? `~${Math.round(f.drive_min)} min drive`
    : `${f.distance_mi} miles`;
  if (f.id === nearest.id) {
    reasons.push(`Closest to you — ${distLabel}`);
  } else if ((f.drive_min ?? f.distance_mi) <= (nearest.drive_min ?? nearest.distance_mi) + 3) {
    reasons.push(`Nearby — ${distLabel}`);
  }

  if (f.rating >= 4.5 && (f.id === topRated.id || f.hospital_name === topRated.hospital_name)) {
    reasons.push(`${f.rating}-star patient rating`);
  }

  if (rank === 1 && hasBookAction(f)) {
    reasons.push("Tap Book on the card to open their scheduling site");
  } else if (rank === 1 && facilityDialPhone(f)) {
    reasons.push(`Call ${f.phoneDepartment || "scheduling"} from the card`);
  }

  if (rank === 1 && f.accredited) {
    reasons.push("Accredited hospital");
  }

  return reasons.slice(0, 3);
}

function badgeForRank(rank: number, f: FacilityResult, all: FacilityResult[]): string | undefined {
...
export function buildScrapeExecutiveSummary(
  profile: PatientProfile,
  facilities: Record<string, FacilityResult>,
  events: ScraperLog[] = []
): ScrapeExecutiveSummary | null {
  ...
  const ranked: RankedOption[] = scored.map(({ f }, i) => ({
    id: f.id!,
    rank: i + 1,
    facility: f,
  ...
  const partialIssues: { name: string; issue: string }[] = [];
  ...
  for (const [id, label] of Object.entries(SOURCE_LABELS)) {
    if (!resultIds.has(id)) {
      missed.push({
        name: label,
        reason: "No public price page found within your search radius",
      });
    }
  }
  ...
  const sourcesChecked = events.length
    ? new Set(events.map((e) => e.collector_id)).size
    : entries.length;
  ...
  if (uniquePartial.length) {
    bullets.push(
      `${uniquePartial.length} site${uniquePartial.length > 1 ? "s" : ""} had delays; data was still recovered where possible`
    );
  }
  if (missed.length) {
    bullets.push(
      `${missed.length} source${missed.length > 1 ? "s" : ""} had no public pricing — listed below`
    );
  }
  ...
  const spokenSummary = [
    ...
    `I checked ${Math.max(sourcesChecked, entries.length)} hospital sites and got clear prices at ${entries.length} of them. ` +
      `With your plan, you're looking at about $${minP} to $${maxP}.`,
    ...
    missed.length
      ? `${missed.length} hospital${missed.length > 1 ? "s" : ""} didn't publish prices online — I've noted those for you.`
      : ``,
    ...
    hasBookAction(recommendation.facility)
      ? `They have online scheduling — tap Book on their card, or ask me to open it.`
      : facilityDialPhone(recommendation.facility)
        ? `To schedule, call their ${recommendation.facility.phoneDepartment || "scheduling line"} — I can dial for you.`
        : ``,
